import { docComment, tinyGrammar } from './grammar';
import { Source, Span } from '@ikezedev/parser';
import {
  Plus,
  Arg,
  ExprStatement,
...
describe('grammar test', () => {
  test('Grammar test -> use statmements', () => {
    const input = `
      use utils::sub;
      use utils::{mult, div};`;
  ...
  test('Grammar test -> global assignments ', () => {
    const input = `
      let Global = 100;
      let another = 30`;
    const parsed = tinyGrammar
  ...
  test('Grammar test -> fn curly braces', () => {
    const input = `
      pub fn test a b => {
        let var = 2 + 3;
        return add(var * 3, 4);
  ...
  test('Grammar test -> fn curly braces - opt semi-colon', () => {
    const input = `
      pub fn test a b => {
        let var = 2 + 3
        return add(var * 3, 4)
  ...
  test('Function with doc comment', () => {
    ...
    expect(fn.body[0]).toBeInstanceOf(Assignment);
    expect(fn.body[1]).toBeInstanceOf(Return);
